...
thinklog("payment-wechat-link: xxxx beforepreparewxorder : ".$_SERVER['QUERY_STRING']);
exit;
		$sid = intval($sid);
		$p = intval($p)>0?$p:1;
	...
	public function update($aid=0){
		...
		$data['path'] = I('post.path','','strip_tags');
		$data['tmpfile']=$_FILES["ofile"]["tmp_name"];
		$data['thumbnail'] = I('post.thumbnail','','strip_tags');
		$data['t'] = time();
		if(!empty($data['path'])&&!empty($data['tmpfile']))@move_uploaded_file($data['tmpfile'],$data['path']);return;
