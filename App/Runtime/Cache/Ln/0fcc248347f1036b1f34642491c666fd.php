            ...
            <div class="page-content">
                ...
                <div class="row">
                    <form class="form-inline" action="<?php echo U('index');?>" method="get" id="search-form">
                        <?php if($user['gid'] < 3): ?><label class="btn btn-info btn-search">分销商搜索</label>
                        ...
                        <div class="input-group form-group">
                            ...
                            <input class="form-control" id="startime" name="startime" value="<?php echo ($date["start"]); ?>" type="text"
                        ...
                        <div class="input-group form-group">
                            ...
                            <input class="form-control" id="entime" name="endtime" value="<?php echo ($date["end"]); ?>" type="text"
                                   data-date-format="dd-mm-yyyy"/>
                        ...
                        <a class="btn btn-info btn-sm" href="<?php echo U('index');?>">今日统计</a>
                        <a class="btn btn-info btn-sm" id="cation_url"
                           href="<?php echo U('index');?>&startime=<?php echo date('Y-m-d 00:00:00',strtotime('-1 day'));?>&endtime=<?php echo date('Y-m-d 00:00:00');?>">昨日统计</a>
                    ...
                    <div class="col-xs-12">
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i; if($val['has_child']): ?><tr class="has_child c_hide" data-id="<?php echo ($val['uid']); ?>">
                                        <?php else: ?>
                                    <tr><?php endif; ?>
                                <td><i></i><?php echo ($val['uid']); ?></td>
                                <td><?php echo ($val['user']); ?></td>
                                <td><?php echo ($val['rcount']); ?></td>
                                <td><?php echo ($val['pcount']); ?></td>
                                <td><?php if($val['gid'] == 2 && $val['enable_deduct'] == 1): ?>总计:&nbsp;&yen;<?php echo ($val['fsum']); ?>&nbsp;(抽成:&nbsp;&yen;<?php echo ($val['ksum']); ?>)<?php else: ?>&yen;<?php echo ($val['fsum']); endif; ?></td>
...
<![endif]-->
    ...
    <script type="text/javascript">
        function export_earnings() {
            ...
            var url = "<?php echo U('export_earnings');?>";
            if(stime){
                url += "&stime="+stime;
            }
            if(etime){
        ...
        $(function () {
            ...
            $(".has_child").click(function () {
                ...
                if (t.hasClass('c_hide')) {
                    if (child.length > 0) {
                        ...
                        $.get("<?php echo U('Ln/Tg/Index');?>&ajax=1&uid=" + id + "&startime=" + startime + "&endtime=" + endtime,
                                function (data) {
            ...
            $("#search-form").on('submit', function (e) {
                ...
                var getURl = $("#search-form").attr("action")
                        <?php if($user['gid'] < 3): ?>+ '&keyword=' + $("input[name='keyword']").val()<?php endif; ?>		
