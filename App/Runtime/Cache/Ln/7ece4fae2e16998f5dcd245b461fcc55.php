            ...
            <div class="page-content">
                ...
                <table class="table table table-striped table-bordered table-hover">
                    ...
                    <tr>
                        <th>时间</th>
                        <th>名称</th>
                        <?php if(is_array($head)): $i = 0; $__LIST__ = $head;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hv): $mod = ($i % 2 );++$i;?><th><?php echo ($hv); ?></th><?php endforeach; endif; else: echo "" ;endif; ?>
                    </tr>
                    </thead>

                    <?php
 $debug = I('get.debug',0); foreach($list as $k => $v){ ?>
                    <tr style="background-color: #ffd3da">
                        <td rowspan="7" style="background: white;"><?= $k;?></td>
                        <td>小时充值</td>
                        <?php foreach($head as $hv){ ?>
                        <td id="xscz<?php echo ($hv); ?>-<?php echo ($k); ?>">
                            <?php echo "<span class='kl'>".$v[$hv]['money']."</span>"; if($debug){ echo "<span class='_red'>(<label>".(int)$v[$hv]['klqmoney']."</label>)</span>扣量<span style='color: blue'>".($v[$hv]['klqmoney']-$v[$hv]['money'])."</span>"; }?></td>
                        <?php }?>
                    </tr>
                    <tr style="background-color: #f9c0f2">
                        ...
                        <td>
                            <?= $v[$hv]['qytmoney'] ;?>
                            <?php
 if($debug){ echo "<span class='_red'>(".(int)$v[$hv]['klqytmoney'].")</span>扣量<span class='_blue'>".($v[$hv]['klqytmoney']-$v[$hv]['qytmoney'])."</span>"; } ?>
                        </td>
                        <?php }?>
                    </tr>
                    <tr style="background-color: #f8eefb">
                        ...
                        <td>
                            ...
                            <?php
 if($debug){ if($v[$hv]['klqytmoney']){ echo "<span class='_red'>(".round( ($v[$hv]['klqmoney']-$v[$hv]['klqytmoney'])/$v[$hv]['klqytmoney'] , 4)*100 . '%'.")</span>"; }else{echo "<span class='_red'>(0)</span>";} } ?>
                        </td>
                        <?php }?>
                    </tr>
                    ...
                    <tr style="background-color: #d6fffb">
                        <td>小时IP</td>
                        <?php foreach($head as $hv){ $ipcount = $iplist[$hv.':'.$k] ? $iplist[$hv.':'.$k] : 0 ; ?>
                        <td><input id="<?php echo ($hv); ?>-<?php echo ($k); ?>" onchange="saveIp(this)" ymd="<?= $hv?>" h="<?= $k?>" type="text" value="<?= $ipcount ;?>" style="width: 100px;text-align: center;" /></td>
                        <?php }?>
                    </tr>
                    ...
                    <tr style="background-color: #d6e1ff">
                        <td >ip转换率</td>
                        <?php foreach($head as $hv){ $ipcount = $iplist[$hv.':'.$k] ? $iplist[$hv.':'.$k] : 0 ; ?>
                        <td id="ipzhl<?php echo ($hv); ?>-<?php echo ($k); ?>" >
                            <?php
 if($ipcount){ echo "<span class='kl'>". round( ($v[$hv]['money']/$ipcount) , 4)*100 . "%</span>" ; }else{ echo "<span class='kl'>0%</span>" ; } ?>
                            <?php
 if($debug ){ if( $ipcount){ echo "<span class='klq _red'>(<label>".round( ($v[$hv]['klqmoney']/$ipcount) , 4)*100 . '%' ."</label>)</span>" ; }else{ echo "<span class='klq _red'>(<label>0%</label>)</span>" ; } } ?>
                        </td>
                        <?php }?>
                    </tr>
                    <tr style="background-color: #daffd6">
                        <td >ip对比前一天</td>
                        <?php foreach($head as $hv){ $todayipcount = $iplist[$hv.':'.$k] ? $iplist[$hv.':'.$k] : 0 ; ?>

                        <td id="dbqytip<?php echo ($hv); ?>-<?php echo ($k); ?>"><?= $v[$hv]['qytip'] ? round( ($todayipcount - $v[$hv]['qytip'])/$v[$hv]['qytip'] , 4)*100 . '%' : '' ; ?></td>
                        <?php }?>
                    </tr>
...
<![endif]-->
    ...
    <script>
        function  saveIp(obj) {
            ...
            $.get("<?php echo U('Ln/Tg/save_ip');?>",{'ymd':ymd,'h':h,'ipcount':ipcount},function (data) {
                if(data == '1'){
                    var idd = $(obj).attr("id");
