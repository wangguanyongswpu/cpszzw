...
require_once "PayNotifyCallBack.class.php";
...
class WxPayApi
{
   ...
   /**
   * 发送红包
   * 必须参数 nonce_str,sign,mch_billno,mch_id,wxappid,send_name,re_openid,total_amount,total_num,wishing
   * client_ip,act_name,remark
   * return 状态码
   **/
   public function push_hongbao($inputObj,$timeOut=6){
       $url= "https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack";
       $inputObj->SetParameter('mch_id',WxPayConfig::MCHID());
       $inputObj->SetParameter('wxappid',WxPayConfig::APPID());
       $inputObj->SetParameter('nonce_str',self::getNonceStr());
       $inputObj->SetParameter('client_ip',$_SERVER['REMOTE_ADDR']);
       $inputObj->SetSign();//签名
       $xml = $inputObj->ToXml();
       $response = self::postXmlCurl($xml, $url, true, $timeOut);
	...
	{		
        ...
        curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,2);//严格校验2
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_CAINFO, WxPayConfig::ROOTCA_PATH()); // CA根证书（用来验证的网站证书是否是CA颁布）
